...
const NEXTJS_SPECIAL_FILES: Record<NextJsSpecialFile, { icon: typeof FileCode; color: string }> = {
  ...
  layout: { icon: Layout, color: "text-violet-400" },
  ...
  template: { icon: Layout, color: "text-pink-400" },
...
function FileTreeItem({ node, depth = 0 }: FileTreeItemProps) {
  ...
  }, [node, isDirectory, setSelectedPath, toggleExpanded, openFile]);
...
function ContextMenu() {
  const { contextMenuPath, contextMenuPosition, closeContextMenu } = useFileTreeStore();
  ...
  const menuItems = [
    { icon: FilePlus, label: "New File", action: () => {} },
    { icon: FolderPlus, label: "New Folder", action: () => {} },
    { type: "divider" as const },
    { icon: Pencil, label: "Rename", action: () => {} },
    { icon: Copy, label: "Copy Path", action: () => navigator.clipboard.writeText(contextMenuPath) },
    { icon: Terminal, label: "Open in Terminal", action: () => {} },
    { type: "divider" as const },
