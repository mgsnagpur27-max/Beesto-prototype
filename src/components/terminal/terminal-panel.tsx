...
export function TerminalPanel() {
  ...
  useEffect(() => {
    ...
    const initTerminal = async () => {
      ...
      terminal.writeln('\x1b[38;5;141m│\x1b[0m   \x1b[1;38;5;183mOrchids Terminal\x1b[0m                       \x1b[38;5;141m│\x1b[0m');
      terminal.writeln('\x1b[38;5;141m│\x1b[0m   \x1b[2mWebContainer runtime v1.6\x1b[0m               \x1b[38;5;141m│\x1b[0m');
      terminal.writeln('\x1b[38;5;141m╰─────────────────────────────────────────╯\x1b[0m');
      terminal.writeln('');
  ...
  const getStatusIndicator = () => {
    switch (status) {
      case 'booting':
        return <span className="text-amber-400 text-xs">Booting...</span>;
      case 'installing':
        return <span className="text-blue-400 text-xs">Installing...</span>;
      case 'starting':
        return <span className="text-violet-400 text-xs">Starting...</span>;
      case 'ready':
  ...
  return (
    <div className="flex h-full flex-col bg-[#0c0c10]">
      <div className="flex h-9 items-center justify-between border-b border-[#1a1a24] px-2">
        <div className="flex items-center">
          <div className="flex h-full items-center gap-2 bg-[#12121a] rounded px-3 py-1.5 text-sm">
            <TerminalIcon className="h-3.5 w-3.5 text-violet-400" />
            <span className="text-xs text-zinc-300">Terminal</span>
            <button 
              onClick={toggleTerminal}
              className="ml-1 rounded p-0.5 text-zinc-500 hover:text-zinc-300 hover:bg-[#1a1a24] transition-colors"
          ...
          <button 
            className="ml-1 rounded p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-[#1a1a24] transition-colors"
            aria-label="New terminal"
          >
            <Plus className="h-3.5 w-3.5" />
          </button>
          <div className="ml-3 px-2 py-0.5 rounded bg-[#12121a]">
            {getStatusIndicator()}
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-[#1a1a24] rounded transition-colors">
      ...
      <div ref={terminalRef} className="flex-1 overflow-hidden bg-[#0a0a0f]">
